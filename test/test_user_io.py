...
class TestUserIO(TestCase):
    def test_do_card(self):
        ...
        self.assertEqual(watson.knowledge_tables[Cards.ROODHART.category][watson.game_state.players[0]][Cards.ROODHART],
                         Knowledge.TRUE)
        for i in range(len(watson.game_state.players)):
            if i > 0:
                self.assertEqual(
                    watson.knowledge_tables[Cards.ROODHART.category][watson.game_state.players[i]][Cards.ROODHART],
                    Knowledge.FALSE)
